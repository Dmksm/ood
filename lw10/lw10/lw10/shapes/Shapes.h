...
class CGroupShape : public IGroupShape, public std::enable_shared_from_this<IStyleEnumerator>
{
private:
	explicit CGroupShape(std::vector<std::shared_ptr<IShape>> shapes)
		: m_shapes(shapes)
		, m_lineStyle(nullptr)
		, m_fillStyle(nullptr)
	{
...
public:
	static std::shared_ptr<IGroupShape> Create(std::vector<std::shared_ptr<IShape>> shapes)
	{
		struct Wrapper : CGroupShape {
			Wrapper(std::vector<std::shared_ptr<IShape>> shapes): CGroupShape(shapes)
			{}
		};
		auto groupShape = std::make_shared<Wrapper>(shapes);
		groupShape->Init();
		return groupShape;
	}
